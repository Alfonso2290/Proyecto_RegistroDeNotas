...
<html>
    <head>
        <title>MANTENIMIENTO TABLA PERSONAL ADMINISTRATIVO</title>
    ...
    <body>
        <form name="form">
            ...
            <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
                ...
                <div class="CabeceraMenus">Mantenimiento Tabla Personal Administrativo</div>
            </nav>   
            <center>
                <div class="DivPrincipalMantenimiento">
                    ...
                    <table class="TablaPrincipalMantenimiento2" >
                        <tr>
                            <td>
                                <button type="button" class="botonNuevo" onclick="NuevoPersonal('<%=request.getContextPath()%>','DepartamentoServlet',1)">
                    ...
                    <div class="container" style="width: 95%">
                        ...
                        <table cellpadding="10" id="miTabla" class="table table-bordered table-striped" border="2"  >
                            <thead>
                                <tr style="background: lightblue;">
                                    <th style="height:50px;">COD.PERSONAL</th>
                                    <th>DNI</th>
                                    <th>PERSONAL</th>
                                    <th>HORA INGRESO</th>
                                    <th>HORA SALIDA</th>
                        ...
                        %>
                                ...
                                <tr>
                                    <td style="height:50px;"><%=obj.getCodigoPersonal()%></td>
                                    <td><%=obj.getDNI()%></td>
                                    ...
                                    <td><%=obj.getCargo()%></td>
                                    <td>
                                        <button type="button" class="botonNuevo" name="btnModificar" onclick="ModificarPersonalAdm('<%=request.getContextPath()%>','PersonalAdministrativoServlet',13,'<%=obj.getCodigoPersonal()%>','<%=obj.getDNI()%>','<%=obj.getHoraIngreso()%>','<%=obj.getHoraSalida()%>','<%=obj.getCargo()%>','<%=obj.getCodigoDepartamento()%>','<%=obj.getCodigoProvincia()%>','<%=obj.getCodigoDistrito()%>','<%=obj.getCodigoCalle()%>','<%=obj.getNombre()%>','<%=obj.getApellidoPaterno()%>','<%=obj.getApellidoMaterno()%>','<%=obj.getSexo()%>','<%=obj.getFechaNac()%>','<%=obj.getCorreo()%>','<%=obj.getTelefono()%>','<%=obj.getNumExt()%>','<%=obj.getNumInt()%>')">
                                            <img src="<%=request.getContextPath()%>/imagenes/actualizar.png" width="20" height="20">
                                            Modificar
                                        </button>
                                    </td>
                                    <td>
                                        <button type="button" class="botonNuevo" name="btnEliminar" onclick="EliminarPersonal('<%=request.getContextPath()%>','PersonalAdministrativoServlet',15,'<%=obj.getCodigoPersonal()%>','<%=obj.getDNI()%>')">
                    ...
                    <table class="TablaPrincipalMantenimiento2">
                        <tr>
                            <td>
                                <center>
                                    <button type="button" class="botonActualizarInfo" onclick="EnlaceLogin('<%=request.getContextPath()%>','PersonalAdministrativoServlet',10)">
